...
static void _MSCRYPTP_trace_log_curl_error_para(
    const char *file,
    const char *func,
    const int line,
...
#define MSCRYPTP_trace_log_curl_error_para(correlationId, flags, title, loc, err, ...) \
    _MSCRYPTP_trace_log_curl_error_para(__FILE__, __FUNCTION__, __LINE__, correlationId, flags, title, loc, err, __VA_ARGS__)

static void _MSCRYPTP_trace_log_curl_error(
    const char *file,
    const char *func,
    const int line,
    const uuid_t correlationId,
    ...
    CURLcode err)
{
    _MSCRYPTP_trace_log_curl_error_para(
        file,
        func,
        line,
        correlationId,
...
#define MSCRYPTP_trace_log_curl_error(correlationId, flags, title, loc, err) \
    _MSCRYPTP_trace_log_curl_error(__FILE__, __FUNCTION__, __LINE__, correlationId, flags, title, loc, err)

static int _curl_ssl_cert_verify_callback(
    X509_STORE_CTX *storeCtx,
    void *arg)
{
    const char *title = MSCRYPTP_CURL_TITLE;
    MSCRYPT_VERIFY_CERT_CTX *ctx = (MSCRYPT_VERIFY_CERT_CTX *) arg;
    ...
    MSCRYPTP_get_verify_cert_ctx_correlationId(ctx, correlationId);
    MSCRYPTP_trace_log(correlationId, MSCRYPTP_TRACELOG_VERBOSE_FLAG, title, "Start");

    ret = MSCRYPTP_X509_verify_cert(
        ctx,
        storeCtx,
        0,              // mscryptFlags
        &verifyChainError);
    ...
    MSCRYPTP_trace_log(correlationId, MSCRYPTP_TRACELOG_VERBOSE_FLAG, title, "Complete");
    return ret;
}
    ...
    const char *engineKeyId)        // Optional, set for client auth
{
    ...
    ret = curl_easy_setopt(curl, CURLOPT_CAPATH, MSCRYPTP_get_default_cert_dir());
    if (ret != CURLE_OK) {
        loc = "CAPATH";
        goto curlErr;
    }
...
curlErr:
    MSCRYPTP_trace_log_curl_error(correlationId, 0, title, loc, ret);
    goto end;
}
